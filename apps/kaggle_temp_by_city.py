...
def evol_temp_par_pays_python() -> pd.DataFrame:
    spark = SparkSession.builder.appName(APP_NAME + ".python").config("spark.driver.memory", "2g").getOrCreate()
    window = Window.partitionBy("Country", "City", "month").orderBy("dt_year")
    df = (
        spark
        .read.csv("/opt/spark-data/global_temp_by_city.csv", header=True, inferSchema=True)
        ...
        .withColumn("prev_year_temp", F.lag("AverageTemperature").over(window))
        .withColumn("prev_dt", F.lag("dt").over(window))
        .withColumn("prev_year", F.lag("dt_year").over(window))
        .withColumn("temp_diff_by_year", (F.col("AverageTemperature") - F.col("prev_year_temp")) / (F.col("dt_year") - F.col("prev_year")))
...
def main():
    ...
    df_sql.to_csv(
        "/opt/spark-data/out_evol_temp_par_pays_python.csv", index=False
    )
    df_py = evol_temp_par_pays_sql()
    df_py.to_csv(
        "/opt/spark-data/out_evol_temp_par_pays_sql.csv", index=False
    )
    # df = df_sql.merge(df_py, how="outer", on=["Country", "City", "dt"]).assign(
    #     diff=lambda _df: _df["temp_diff_by_year_x"] - _df["temp_diff_by_year_y"]
    # )
    # df[df["diff"].abs() > 0.00001].to_csv("/opt/spark-data/out_diff.csv", index=False)
